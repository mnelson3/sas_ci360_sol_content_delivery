# sas_ci360_sol_content_delivery

Historical Python client for SAS Customer Intelligence 360 **content-delivery** (digital assets) APIs, retained as a reference. It is part of the `sas-ci360` family of solution packages, alongside packages for planning, identity, execution, data, and workflow.
...
`sasci360solcontentdelivery` wraps the CI 360 digital assets REST API with a thin `ContentDelivery` class built on top of the shared `sasci360apicore` (connection, encryption, logging) and `sasci360apidigitalassets` packages. It's meant to be dropped into automation scripts or scheduled jobs that need to manage CI 360 content-delivery/digital assets programmatically rather than through the UI.
...
Key dependencies: `pandas`, `PyJWT`, `requests`, `saspy`, `schedule`, `DateTime`, `urllib3`, plus the sibling `sasci360apicore` and `sasci360apidigitalassets` packages.
...
```python
from sasci360solcontentdelivery import ContentDelivery
